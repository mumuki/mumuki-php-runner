...
class Mumukit::Server::App < Sinatra::Base
  ...
  def self.get_board_asset(route, path, type)
    get_asset route, Php::Board.assets_path_for(path), type
  end

  def self.get_editor_asset(route, path, type)
    get_asset route, Php::Blockly.assets_path_for(path), type
  end

  def self.get_submit_asset(route, path, type)
    get_asset route, Php::CodeRunner.assets_path_for(path), type
  end
  ...
  get_submit_asset 'editor/php-code-runner.html', 'htmls/php-code-runner.html', 'text/html'
