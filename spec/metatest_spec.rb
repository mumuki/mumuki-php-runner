...
describe 'metatest' do
  ...
  let(:framework) do
    Mumukit::Metatest::Framework.new checker: Php::Checker.new(options),
                                     runner: Php::MultipleExecutionsRunner.new
  end
  ...
  before { allow(Mumukit).to receive(:runner_url) { 'http://php.runners.mumuki.io' } }
